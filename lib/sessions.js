...
function parseClaudeSession(filePath, sessionId, projectKey) {
  ...
  // Derive project name and full path from project key
  // Key format: "-Users-geert-code-agenttalktome" → path: "/Users/geert/code/agenttalktome"
  const projectName = projectKey.split("-").filter(Boolean).pop() || projectKey;
  const projectPath = "/" + projectKey.split("-").filter(Boolean).join("/");
