...
/**
 * Connect to ListenerChannel and wait for subscription confirmation.
 * Returns a promise that resolves with the CableConnection once subscribed,
 * or rejects after a timeout.
 */
export function connectListenerChannelAsync(baseUrl, token, onMessage, timeoutMs = 10_000) {
  const connection = connectListenerChannel(baseUrl, token, onMessage);

  return new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(new Error("ListenerChannel subscription timed out"));
    }, timeoutMs);
    ...
    connection.on("rejected", () => {
      clearTimeout(timer);
      reject(new Error("ListenerChannel subscription rejected"));
    });
  });
}
